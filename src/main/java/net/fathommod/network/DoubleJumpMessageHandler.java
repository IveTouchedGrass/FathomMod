...
public class DoubleJumpMessageHandler {
    ...
    public static void handleOnServer(final DoubleJumpMessage.DoubleJumpPacket data, final IPayloadContext context) {
        try {
            Player entity = context.player();
            if ((DevUtils.hasTrinket(entity, Trinkets.DOUBLE_JUMP) || DevUtils.hasTrinket(entity, Trinkets.DOUBLE_DOUBLE_JUMP_AND_JUMP_HEIGHT_ON_CRACK_BRO_THIS_IS_SO_OVERPOWERED_AND_TRASH_AT_THE_SAME_TIME_BECAUSE_YOU_TAKE_HELLA_FALL_DAMAGE)) && entity.getData(FathommodModVariables.ENTITY_VARIABLES).doubleJumpCooldownInt == 0 && (!entity.getData(FathommodModVariables.ENTITY_VARIABLES).doubleJumpCooldown || (DevUtils.hasTrinket(entity, Trinkets.DOUBLE_DOUBLE_JUMP_AND_JUMP_HEIGHT_ON_CRACK_BRO_THIS_IS_SO_OVERPOWERED_AND_TRASH_AT_THE_SAME_TIME_BECAUSE_YOU_TAKE_HELLA_FALL_DAMAGE) && !entity.getData(FathommodModVariables.ENTITY_VARIABLES).secondDoubleJumpUsed)) && !entity.isFallFlying() && !entity.onClimbable() && !entity.isInWater()) {
