...
public class AutoAttackPacketHandler {
    public static void handleDataOnServer(final AutoAttackMessage.AutoAttackPacket data, final IPayloadContext context) {
        ...
        if ((vars.lastAutoAttack > 3 || !(entity.getItemBySlot(EquipmentSlot.MAINHAND).getItem() instanceof DamageTypedWeapon weapon && weapon.getDamageClass() == DamageClasses.ASSASSIN)) && DevUtils.hasTrinket(entity, Trinkets.CHAINED_HANDLE) && entity.getAttackStrengthScale(0.5f) >= 1f) {
