...
public class AutoAttackPacketHandler {
    public static void handleDataOnServer(final AutoAttackMessage.AutoAttackPacket ignored, final IPayloadContext context) {
        ...
        if ((!(entity.getItemBySlot(EquipmentSlot.MAINHAND).getItem() instanceof DamageTypedWeapon weapon && weapon.getDamageClass() == DamageClasses.ASSASSIN) || vars.lastAutoAttack > weapon.getIFrames() / 2) /*&& DevUtils.hasTrinket(entity, Trinkets.AUTO_ATTACK)*/ && entity.getAttackStrengthScale(0f) >= 1f) {
            ...
            Entity entityToAttack = DevUtils.performPreciseRaycast(entity, entity.level(), pos, entity.getLookAngle(), Objects.requireNonNull(entity.getAttribute(Attributes.ENTITY_INTERACTION_RANGE)).getValue());
            if (entityToAttack != null) {
                entity.attack(entityToAttack);
            }
            entity.swing(InteractionHand.MAIN_HAND, true);
            entity.resetAttackStrengthTicker();
            if (Objects.requireNonNull(entity.getAttribute(Attributes.ATTACK_SPEED)).getValue() < 20)
