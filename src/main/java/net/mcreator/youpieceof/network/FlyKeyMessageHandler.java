...
public class FlyKeyMessageHandler {
    public static void handleDataOnServer(final FlyMessage.FlyKeyMessage data, final IPayloadContext context) {
        ...
        if (entity.getItemBySlot(EquipmentSlot.HEAD).getItem() == TrinketDict.WINGS && entity.getData(FathommodModVariables.PLAYER_VARIABLES).doubleJumpCooldownInt <= 0 && entity.getData(FathommodModVariables.PLAYER_VARIABLES).doubleJumpCooldownInt == 0 && !entity.onGround()) { // gold helmet is a placeholder
            if (!(damage + 1 == maxDamage)) {
                entity.getItemBySlot(EquipmentSlot.HEAD).hurtAndBreak(1, entity, EquipmentSlot.HEAD);
                PacketDistributor.sendToPlayer((ServerPlayer) entity, new FlyMessage.FlyKeyMessage(true, (DevUtils.hasTrinket(entity, TrinketDict.DOUBLE_DOUBLE_JUMP_AND_JUMP_HEIGHT_ON_CRACK_BRO_THIS_IS_SO_OVERPOWERED_AND_TRASH_AT_THE_SAME_TIME_BECAUSE_YOU_TAKE_HELLA_FALL_DAMAGE)) ? 4 : (DevUtils.hasTrinket(entity, TrinketDict.JUMP_HEIGHT)) ? 2.5 : 1));
            } else {
                PacketDistributor.sendToPlayer((ServerPlayer) entity, new FlyMessage.FlyKeyMessage(false, (DevUtils.hasTrinket(entity, TrinketDict.DOUBLE_DOUBLE_JUMP_AND_JUMP_HEIGHT_ON_CRACK_BRO_THIS_IS_SO_OVERPOWERED_AND_TRASH_AT_THE_SAME_TIME_BECAUSE_YOU_TAKE_HELLA_FALL_DAMAGE)) ? 4 : (DevUtils.hasTrinket(entity, TrinketDict.JUMP_HEIGHT)) ? 2.5 : 1));
            }
        }
    }
